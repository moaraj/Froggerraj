...
const GameUtilities = /** @class */ (function () {
  ...
  GameUtilities.prototype.randomizeLocation = function (spawnLocationX, spawnLocationY) {
    ...
    const randX = Math.floor(Math.random() * spawnLocationX) * 101;
    const randY = Math.floor(Math.random() * spawnLocationY) * 85 + 40;
    return [randX, randY];
  };
...
const gameUtils = new GameUtilities(0, 1);
const genCharacterCardDeck = function () {
  // produce divs for character cards, example given below
  // <div class="character-card"><img id="boy" src="./images/char-boy.png"></div>
  const characterCardDeck = document.getElementById('character-deck');
...
const characterDeck = document.getElementById('character-deck');
if (characterDeck) {
...
function changeCharCardColor(cardElement) {
  ...
  characterDeck.childNodes.forEach((box) => {
    if (cardElement === box) { box.style.background = 'yellow'; } else if (box.style.background = '#2e3d49') ;
  });
}
...
const GameObject = /** @class */ (function () {
...
const FloatingObject = /** @class */ (function (_super) {
  __extends(FloatingObject, _super);
  function FloatingObject(name, x, y, radius, sprite, points) {
    const _this = _super.call(this, name, x, y, radius, sprite, points) || this;
    _this.xInit = x;
    _this.yInit = y;
    _this.radius = radius;
...
const WinningBlock = /** @class */ (function (_super) {
  ...
  WinningBlock.prototype.checkPlayerInsideWinBlock = function () {
    ...
    const insideBlock = Math.sqrt(Math.pow((player.x - this.x), 2) + Math.pow((player.y - this.y), 2)) <= this.radius;
    return insideBlock;
  };
...
const winPad = new WinningBlock('winningBlock', 1000, 40, 20, gameUtils.objectSprites.selector, 20);
...
const genFloatingGameObjects = function (name, gameObjectSprite, spawnLocationX, spawnLocationY, rad, points) {
  // Function generates Game objects with random x and y block placements
  // and tries to get them not to overlap
  const _a = gameUtils.randomizeLocation(spawnLocationX, spawnLocationY); const xObj = _a[0]; let
    yObj = _a[1];
  const _b = getObjectCoords('key'); const keyX = _b[0]; const
    keyY = _b[1];
...
const genGameObjects = function () {
  // Clear the Floating Objects array and pl;ayer inventory
  // and generate new key and gem instances in random locations
  floatingGameObjects = [];
  const winKey = genFloatingGameObjects('key', gameUtils.objectSprites.keypic, 4, 4, 40, 10);
  floatingGameObjects.push(winKey);
  const blueGem = genFloatingGameObjects('blueGem', gameUtils.objectSprites.gemBlue, 3.5, 4.5, 55, 10);
  floatingGameObjects.push(blueGem);
  const greenGem = genFloatingGameObjects('greenGem', gameUtils.objectSprites.gemGreen, 5, 4, 55, 10);
...
function gameStartGenEnemies(difficulty) {
  ...
  for (let index = 0; index < 4; index++) {
    const xInit = index * Math.random() * 400;
    const yInit = index * 80 + 50;
    const speedInit = Math.random() * 4 + difficulty;
...
function genEnemies(xInit, yInit, speedInit) {
  ...
  const newEnemy = new Enemy(xInit, yInit, speedInit);
...
function genEnemiesProb(yLevels, speedMax, prob, maxEnemies) {
  ...
  const genEnemyProb = Math.floor(Math.random() * 100) > prob + allEnemies.length;
  // console.log(prob + allEnemies.length * 2)
  if (genEnemyProb && allEnemies.length < maxEnemies) {
    // console.log('bug generated');
    const yInitEnemy = Math.floor(Math.random() * yLevels * 2) * 40 + 50;
    const speedInitEnemy = Math.random() * speedMax + 1;
    const newEnemy = new Enemy(-100, yInitEnemy, speedInitEnemy);
    allEnemies.push(newEnemy);
    gameUtils.arrangeObjectsByY(allEnemies);
  }
}
...
function deleteEnemiesProb(prob) {
  ...
  allEnemies.forEach((bug, index) => {
    if (bug.x > 500 && allEnemies.length > 7) {
      const delEnemyProb = Math.floor(Math.random() * 100) > prob - allEnemies.length * 5;
      if (delEnemyProb) {
...
const detectNearbyEnemies = function (enemyArray, yThresholdTop, yThresholdBottom, xThreshold) {
...
const collisionDetection = function () {
  ...
  const nearbyEnemies = detectNearbyEnemies(allEnemies, 70, 70, 100);
  const collidingEnemies = detectNearbyEnemies(nearbyEnemies, 30, 50, 57);
  // If collision is detected Player health and lives are updated
  if (collidingEnemies.length > 0) { updatePlayerHealth(); }
};
const detectOtherBugs = function () {
  // Figure out which bugs have bugs in the same lane
  allEnemies.forEach((thisBug) => {
    const bugsFollowingInLane = allEnemies.filter(bug => bug.y === thisBug.y).filter(bug => bug.x < thisBug.x);
...
const updatePlayerHearts = function () {
  ...
  const hud = document.getElementsByClassName('hud-status').item(0);
  ...
  const range = Array(player.totalLives).keys().slice();
  range.forEach((index) => {
...
const checkGameFinish = function () {
  checkGameWin();
  if (player.isDead) {
    document.getElementById('end-screen-text').innerText = "Your hero has fallen. Prehaps he shall rise again after a good night's rest.";
    gameWinSequence();
  } else if (player.win) {
    document.getElementById('end-screen-text').innerText = 'Your hero is vicotrious. Thou shalt live like a king';
    gameWinSequence();
  }
};
